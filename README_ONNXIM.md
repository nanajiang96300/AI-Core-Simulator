...
(docker) ./build/bin/Simulator --config ./configs/systolic_ws_128x128_c4_simple_noc_tpuv4.json --models_list ./example/models_list.json
```
...
```
$ cd ..
$ ./build/bin/Simulator --config ./configs/systolic_ws_128x128_c4_simple_noc_tpuv4.json --models_list ./example/models_list.json
```
...
## 3. Test Quick Start
You can run unit/integration tests to quickly inspect the simulator execution flow.
...
$ conan install ..
$ cmake ..
$ cmake --build . -j --target Simulator_test
```

### Run all tests
```
$ ctest -R Simulator_test --output-on-failure
```

### Run a single test (example)
```
$ ./Simulator_test --gtest_filter=SystolicWSTileExecutionTest.BasicAssertions
```

Recommended files to read while running tests:
- `tests/SystolicWsTest.cc`: core issue/cycle and memory request-response loop
- `tests/MappingTest.cc`: mapping parser behavior
- `src/main.cc` and `src/Simulator.cc`: end-to-end model registration and cycle loop
